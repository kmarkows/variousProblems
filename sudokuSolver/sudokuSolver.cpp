...
bool SudokuSolver::solveRecurrsively(uint8_t startRow, uint8_t startCol)
{
    ...
    std::bitset<sudokuSize> currentPosBits = rowsBits[nextEmptyPosition.first] |
        columnsBits[nextEmptyPosition.second] | cellsBits[getCellId(nextEmptyPosition.first, nextEmptyPosition.second)];
